...
def store_data(**kwargs):
    ...
    if not data or "error" in data:
        raise ValueError("Invalid data format returned from fetch_air_quality_data")

    record_time_str = data["timestamp"]
    ...
    insert_query = """
        INSERT INTO aqi_data_new (timestamp, pm25_aqi, pm10_aqi, so2_aqi, no2_aqi, nh3_aqi) 
        VALUES (%s, %s, %s, %s, %s, %s)
    """
    cursor.execute(insert_query, (timestamp, pm25_aqi, pm10_aqi, so2_aqi, no2_aqi, nh3_aqi))

    conn.commit()
    cursor.close()
    conn.close()

    print("Data inserted successfully:", timestamp, pm25_aqi, pm10_aqi, so2_aqi, no2_aqi, nh3_aqi)
...
dag = DAG(
    ...
    description="A DAG to fetch air quality data every 15 minutes",
